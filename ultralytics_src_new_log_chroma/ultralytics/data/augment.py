...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels):
        ...
        for i in range(3):
            ...
            # Place img in img3
            if i == 0:  # center
                _fv = 114 if img.dtype == np.uint8 else (29298 if img.dtype == np.uint16 else 114)
                img3 = np.full((s * 3, s * 3, img.shape[2]), _fv, dtype=img.dtype)
                h0, w0 = h, w
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                _fv = 114 if img.dtype == np.uint8 else (29298 if img.dtype == np.uint16 else 114)
                img4 = np.full((s * 2, s * 2, img.shape[2]), _fv, dtype=img.dtype)
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            if i == 0:  # center
                _fv = 114 if img.dtype == np.uint8 else (29298 if img.dtype == np.uint16 else 114)
                img9 = np.full((s * 3, s * 3, img.shape[2]), _fv, dtype=img.dtype)
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
...
class RandomPerspective:
    ...
    def affine_transform(self, img, border):
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                _fv = 114 if img.dtype == np.uint8 else (29298 if img.dtype == np.uint16 else 114)
                img = cv2.warpPerspective(img, M, dsize=self.size, borderValue=(_fv, _fv, _fv))
            else:
                _fv = 114 if img.dtype == np.uint8 else (29298 if img.dtype == np.uint16 else 114)
                img = cv2.warpAffine(img, M[:2], dsize=self.size, borderValue=(_fv, _fv, _fv))
        return img, M, s
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            if img.dtype == np.uint16:
                tmp = (img.astype(np.float32) / 65535.0 * 255.0).astype(np.uint8)
                ...
                lut_sat[0] = 0
                h, s, v = cv2.split(cv2.cvtColor(tmp, cv2.COLOR_BGR2HSV))
                hsv = cv2.merge((cv2.LUT(h, lut_hue), cv2.LUT(s, lut_sat), cv2.LUT(v, lut_val)))
                cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR, dst=tmp)
                labels["img"] = (tmp.astype(np.uint16) * 257)
            else:
                dtype = img.dtype
                r = np.random.uniform(-1, 1, 3) * [self.hgain, self.sgain, self.vgain]
                x = np.arange(0, 256, dtype=r.dtype)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if c == 3:
            _fv = 114 if img.dtype == np.uint8 else (29298 if img.dtype == np.uint16 else 114)
            img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(_fv, _fv, _fv))
        else:  # multispectral
            _fv = 114 if img.dtype == np.uint8 else (29298 if img.dtype == np.uint16 else 114)
            pad_img = np.full((h + top + bottom, w + left + right, c), fill_value=_fv, dtype=img.dtype)
...
class Format:
    ...
    def _format_img(self, img):
        ...
        if img.dtype == np.uint16:
            ...
            if m > 1.0:
                img = img / (65535.0 if m > 255.0 else 255.0)
...
class ToTensor:
    ...
    def __call__(self, im):
        ...
        x = np.ascontiguousarray(x[::-1] if x.shape[0] == 3 else x)
        ...
        if dt == np.uint16:
            ...
            m = float(x.max()) if hasattr(x, "max") else 1.0
            if m > 1.0:
                t = t / (65535.0 if m > 255.0 else 255.0)
        return t
