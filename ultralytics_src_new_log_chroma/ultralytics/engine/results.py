...
class Results(SimpleClass):
    ...
    def plot(
        ...
        _img = self.orig_img if img is None else img
        if isinstance(_img, np.ndarray):
            if _img.dtype == np.uint16:
                m = float(_img.max())
                mi = float(_img.min())
                if m > mi:
                    _img = (((_img - mi) * (255.0 / (m - mi + 1e-6))).clip(0, 255)).astype(np.uint8)
                else:
                    _img = _img.astype(np.uint8)
            elif _img.dtype in (np.float32, np.float64):
                m = float(np.max(_img))
                if m > 0:
                    _img = (_img * (255.0 / m)).clip(0, 255).astype(np.uint8)
                else:
                    _img = (_img * 255.0).clip(0, 255).astype(np.uint8)
        annotator = Annotator(
            deepcopy(_img),
            line_width,
            font_size,
