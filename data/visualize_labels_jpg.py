...
def main():
    ...
    # NEW: YOLO txt for JPGs
    p.add_argument(
        '--labels',
        type=str,
        default="/projects/SuperResolutionData/carolinali-shadowRemoval/data/log_chroma_jpg_txt"
    )
    # NEW: visualization output for JPGs
    p.add_argument(
        '--out',
        type=str,
        default="/projects/SuperResolutionData/carolinali-shadowRemoval/data/log_chroma_jpg_vis"
    )
    p.add_argument(
        '--names',
        type=str,
        nargs='*',
        default=['Car','Cyclist','Pedestrian','Tram','Truck']
    )
    args = p.parse_args()
