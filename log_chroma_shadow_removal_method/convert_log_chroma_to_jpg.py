...
def find_image(image_dir, base):
    """
    Look for base.jpg / base.jpeg / base.png
    """
...
def convert(label_dir, image_dir, out_dir):
    ...
    for i, f in enumerate(files):
        ...
        base = Path(f).stem
        imgp = find_image(image_dir, base)

        if imgp is None:
            print(f"[WARN] No JPG found for {base}, skipping...")
            continue

        # use actual JPG size
        h, w = get_image_size(imgp)
...
def main():
    ...
    args = p.parse_args()
    convert(args.label_dir, args.image_dir, args.out_dir)
